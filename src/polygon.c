 ...
 */
struct polygon_t *
Stds_CreatePolygon( const int32_t sides, const float scale, const struct vec2_t position,
                    const float angle ) {
  ...
  float f_theta     = ( PI * 2.f ) / polygon->sides;
  polygon->position = position;
  polygon->angle    = angle;

  for ( int32_t i = 0; i < polygon->sides; i++ ) {
    struct vec2_t data =
        Stds_CreateVec2( 20.0f * cosf( f_theta * i ) * scale, 20.0f * sinf( f_theta * i ) * scale );
...
Stds_UpdatePolygon( struct polygon_t *polygon ) {
  for ( int32_t i = 0; i < polygon->sides; i++ ) {
    ...
    polygon->points[i].x = ( polygon->model[i].x * cosf( polygon->angle ) ) -
                           ( polygon->model[i].y * sinf( polygon->angle ) ) + polygon->position.x;
    polygon->points[i].y = ( polygon->model[i].x * sinf( polygon->angle ) ) +
                           ( polygon->model[i].y * cosf( polygon->angle ) ) + polygon->position.y;
    polygon->overlap = false;
  }
}
